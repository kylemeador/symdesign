...
def predict_structure(job: PoseJob):
...
@remove_structure_memory
@handle_design_errors(errors=(DesignError,))
def custom_rosetta_script(job: PoseJob, script, file_list=None, native=None, suffix=None,
...
@remove_structure_memory
@handle_design_errors(errors=(DesignError,))
def interface_metrics(job: PoseJob):
    ...
    # ANALYSIS: each output from the Design process based on score, Analyze Sequence Variation
    if not job.job.distribute_work:
        pose_s = job._interface_design_analysis()
        out_path = os.path.join(job.job.all_scores, putils.default_analysis_file.format(starttime, 'All'))
        if os.path.exists(out_path):
            header = False
        else:
...
@remove_structure_memory
@handle_design_errors(errors=(DesignError,))
def check_unmodelled_clashes(job: PoseJob, clashing_threshold: float = 0.75):
...
def orient(job: PoseJob, to_pose_directory: bool = True):
...
@remove_structure_memory
@handle_design_errors(errors=(DesignError,))
def generate_fragments(job: PoseJob):
...
@remove_structure_memory
@handle_design_errors(errors=(DesignError,))
def interface_design(job: PoseJob):
...
@remove_structure_memory
@handle_design_errors(errors=(DesignError,))
def optimize_designs(job: PoseJob, threshold: float = 0.):
    ...
    job.log.info(f'{job.optimize_designs.__name__} command: {list2cmdline(design_cmd)}')
    metric_cmds = []
    metric_cmds.extend(job.generate_entity_metrics(entity_cmd))
...
@remove_structure_memory
@handle_design_errors(errors=(DesignError,))
def process_rosetta_metrics(job: PoseJob):
...
@remove_structure_memory
@handle_design_errors(errors=(DesignError,))
def interface_design_analysis(job: PoseJob, designs: Iterable[Pose] | Iterable[AnyStr] = None) \
        -> pd.Series:
...
@handle_design_errors(errors=(DesignError,))
def select_sequences(job: PoseJob, filters: dict = None, weights: dict = None, number: int = 1, protocols: list[str] = None,
                     **kwargs) -> list[str]:
...
class PoseDirectory(PoseProtocol):
    ...
    @classmethod
    def from_file(cls, design_path: str,  project: str = None, **kwargs):
        source_path = design_path = os.path.abspath(design_path)
        if not os.path.exists(design_path):
            raise FileNotFoundError(f'The specified Pose source "{design_path}" was not found!')
        filename, extension = os.path.splitext(design_path)
        ...
        if project is None:
            ...
            # except IndexError:  # We only have a 2 index list
                raise InputError(f"Couldn't get the project from the path '{design_path}'. Please provide "
                                 f"project name with --{flags.project_name}")

        return cls(design_path, initialized=False,  # source_path=source_path,
    ...
    def __init__(self, source_path: AnyStr, name: str = None, project: str = None,  # design_path: AnyStr,
        ...
        # self.source = self.source_path = os.path.abspath(design_path)
        ...
        if not self.initialized:
            ...
            self.path = os.path.join(self.job.projects, self.project, self.name)
        ...
        # PoseJob path attributes. Set after finding correct path
        self.log_path: str | Path = os.path.join(self.path, f'{self.name}.log')
...
class PoseProtocol(PoseData):
    ...
    def interface_design_analysis(self, designs: Iterable[Pose] | Iterable[AnyStr] = None) -> pd.Series:
        ...
        if self.job.figures:  # For plotting collapse profile, errat data, contact order
            ...
            fig.tight_layout()
            fig.savefig(os.path.join(self.data, 'DesignMetricsPerResidues.png'))  # Todo PoseJob(.path)
    ...
    def analyze_residue_metrics_per_design(self, designs: Iterable[Pose] | Iterable[AnyStr] = None) -> pd.DataFrame:
        """Perform per-residue analysis on designs and write to the PoseJob residues.csv
        ...
        per_residue_data: dict[str, dict[str, Any]] = {}
        # if designs is None:
        #     measure_structure = False
        #     designs = [Pose.from_file(file, **self.pose_kwargs) for file in self.get_designs()]
        #     # else:
        #     #     measure_structure = False
        #     designs = []
        # else:
        ...
        #     # #
        #     # # self.log.debug(f'Viable designs with structures remaining after cleaning:\n\t{", ".join(viable_designs)}')
        #     rosetta_info_df = self.process_rosetta_metrics()
        #     # Merge interface design specific residue metrics with total per residue metrics
        #     # residues_df = pd.merge(residues_df, rosetta_info_df, left_index=True, right_index=True)
        #     residues_df = residues_df.join(rosetta_info_df)
        # else:
    ...
    def analyze_design_metrics_per_design(self, residues_df: pd.DataFrame, designs_df: pd.DataFrame = None,
        ...
        designs_df.to_csv(self.designs_metrics_csv)  # Todo PoseJob(.path)
