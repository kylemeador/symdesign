...
def create_trajectory(design_directories, name='docking_trajectory', output_dir=os.getcwd()):
    ...
    for des_dir in design_directories:
        trajectory_model.append(merge_pose_pdbs(des_dir))
...
def merge_pose_pdbs(des_dir, frags=True):
    ...
    if frags:
        frag_pdbs = glob(os.path.join(des_dir.frags_path, '*.pdb'))
        frags = []
...
if __name__ == '__main__':
    ...
    all_design_directories = [protocols.protocols.PoseJob.from_nanohedra(design_path, symmetry=args.design_string)
