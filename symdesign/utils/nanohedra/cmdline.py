    ...
    if '-entry' in arg_list:
        ...
        if entry_index < len(arg_list):
            if arg_list[entry_index].isdigit() and \
                    (int(arg_list[entry_index]) in range(1, len(utils.SymEntry.nanohedra_symmetry_combinations))):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    utils.nanohedra.postprocess.frag_match_count_filter(design_dir_path, min_matched, outdir)
                except ValueError:
                    logfile = open(log_filepath, "w")
