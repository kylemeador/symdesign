...
def rand_perturb(pdb1_path, pdb2_path, set_matrix1, set_matrix2, rot_degen_matrix1, rot_degen_matrix2, internal_tx1,
    ...
    for perturb_number in range(number_of_perturbations):
        ...
        if ref_frame_tx_dof1 != '<0,0,0>':
            ext_tx_vector1 = utils.SymEntry.get_optimal_external_tx_vector(ref_frame_tx_dof1, perturbed_ext_dof_shifts)
            pdb1_copy.translate(ext_tx_vector1)

        if ref_frame_tx_dof2 != '<0,0,0>':
            ext_tx_vector2 = utils.SymEntry.get_optimal_external_tx_vector(ref_frame_tx_dof2, perturbed_ext_dof_shifts)
        ...
        if uc_spec_string != 'N/A':
            ...
            uc_dimensions = utils.SymEntry.get_uc_dimensions(uc_spec_string, _e_perturbed, _f_perturbed, _g_perturbed)
            cryst1_record = utils.symmetry.generate_cryst1_record(uc_dimensions, result_design_sym)
