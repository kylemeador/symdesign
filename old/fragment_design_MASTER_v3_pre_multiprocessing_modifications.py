...
flags = ['-ex1', '-ex2', '-linmem_ig 10', '-ignore_unrecognized_res', '-ignore_zero_occupancy false', '-overwrite',
         '-extrachi_cutoff 5', '-out:path:pdb rosetta_pdbs/', '-out:path:score scores/',
...
refine_options = ['-constrain_relax_to_start_coords', '-use_input_sc', '-relax:ramp_constraints false', '-no_optH false'
                  , '-relax:coord_constrain_sidechains', '-relax:coord_cst_stdev 0.5', '-no_his_his_pairE', '-flip_HNQ',
...
design_options = ['-use_occurrence_data', '-mute all', '-unmute protocols.rosetta_scripts.ParsedProtocol',
                  '-out:suffix _design']
                  # -holes:dalphaball
design_stage = ['refine', 'design', 'filter']
# initiate_refine = '%s.sh' % design_stage[0]
# initiate_design = '%s.sh' % design_stage[1]
index_offset = -1
...
def combine_ssm(pssm, issm, cluster_mapping, weights=False):
    # Combine weights for the PSSM with the ISSM
    # HHblits PSSM Input: {0: {'A': 0.13, ..., 'log': [-5, ...], 'type': 'W', 'info': 0.00, 'weight': 0.00}, {...}}
    # PSIBLAST PSSM Input: {0: {'A': 0.13, ..., 'log': [-5, ...], 'type': 'W', 'info': 3.20, 'weight': 0.73}, {...}}
...
def parse_psi_pssm(file, pose_dict):
    ...
    for residue in pssm:
        ...
        for aa in pose_dict[resi]:
            pose_dict[resi][aa] = (residue[3][i] / 100.0)
            i += 1
...
def write_shell_script(command, i):
    with open(design_stage[i] + '.sh', 'w') as f:
        f.write('#!/bin/bash\n\n')
        f.write('%s &&\n' % command)
        f.write('bash %s\n' % design_stage[i + 1] + '.sh')


def main(design_directory, frag_db, sym, debug=False):  # script=False
    # Variable initialization
    done_process = subprocess.Popen(start_cmd)
    # Design_directory always has format:
    #   design_symmetry/building_blocks/DEGENX_X/ROT_Y/tx_Z/
    # ex: P432/4ftd_5tch/DEGEN1_2/ROT_1/tx_2
    # sym_dir backs out to design_symmetry (P432)
    sym_dir = design_directory[:design_directory.find(design_directory.split('/')[-4]) - 1]
    design_sequence_dir = os.path.join(sym_dir, sequence_info)
    if not os.path.exists(design_sequence_dir):
        os.makedirs(design_sequence_dir)
    # base_dir backs out to building_blocks (4ftd_5tch)
    ...
    paths = []
    # name = os.path.basename(base_dir.upper().split('_'))  #GOTIME
    for root, dirs, files, in os.walk(design_directory):
        for file in files:
            ...
            # elif fnmatch.fnmatch(file, name[0] + '*'):  #GOTIME
            #     oligomer1 = read_pdb(os.path.join(root, file))
            #     oligomer1_com = oligomer1.center_of_mass()
            #     # olig_seq1, error_dump1 = fc.extract_aa_seq(oligomer1)
            # elif fnmatch.fnmatch(file, name[1] + '*'):  # GOTIME
            #     oligomer2 = read_pdb(os.path.join(root, file))
            #     oligomer2_com = oligomer2.center_of_mass()
            #     # olig_seq2, error_dump2 = fc.extract_aa_seq(oligomer2)

            if fnmatch.fnmatch(file, '*_parsed_orient_tx_*'):  # '*_tx_*' Will be depreciated once full SymDock is ready
                paths.append((root, file))
                if len(paths) == 2:
                    oligomer1 = read_pdb(os.path.join(paths[0][0], paths[0][1]))
                    oligomer1_name = str(os.path.basename(oligomer1.filepath).split('_')[0])
                    oligomer1_com = oligomer1.center_of_mass()
                    olig_seq1, error_dump1 = fc.extract_aa_seq(oligomer1)
                    oligomer2 = read_pdb(os.path.join(paths[1][0], paths[1][1]))
                    oligomer2_name = str(os.path.basename(oligomer2.filepath).split('_')[0])
                    oligomer2_com = oligomer2.center_of_mass()
                    olig_seq2, error_dump2 = fc.extract_aa_seq(oligomer2)
                    int_res_atoms1, int_res_atoms2 = find_interface_residues(oligomer1, oligomer2)

            # TODO insert mechanism to gather my own fragment decoration statistics
    ...
    # print(module, 'Input PDBs:', name[0].upper(), name[1].upper()) #GOTIME
    print(module, 'Input PDBs:', oligomer1_name.upper(), oligomer2_name.upper())
    ...
    refined_pdb = os.path.join(design_directory, rosetta_pdbs_outdir, os.path.splitext(ala_mut_refine_pdb)[0]
                               + refine_suffix + '_0001.pdb')
    ...
    print(module, 'Jump Site:', jump)  # TODO only need the designation 1 as there is only 1 jump. Residue not important
    ...
    # REFINE: Prepare Command, and Flags File then run RosettaScripts on Cleaned ASU #OPTIMIZE #GOTIME REMOVE
    if not debug:
        if not os.path.exists(refined_pdb):
            refine_variables = [('pdb_reference', cleaned_pdb), ('dist', dist)]
            flagrefine = prepare_rosetta_flags(refine_options, refine_variables, design_stage[0])
    ...
    # # REFINE: Prepare Command, and Flags File then run RosettaScripts on Cleaned ASU #OPTIMIZE #GOTIME
    # refine_variables = [('pdb_reference', cleaned_pdb), ('dist', dist)]
    # flagrefine = prepare_rosetta_flags(refine_options, refine_variables, design_stage[0])
    # input_refine_cmds = ['-in:file:s', ala_mut_refine_pdb, '@' + flagrefine, '-parser:protocol', protocol[0]]
    # refine_cmd = prepare_rosetta_scripts_cmd(input_refine_cmds)
    # if script:
    #     write_shell_script(subprocess.list2cmdline(refine_cmd), i=0)
    #     refine_process = done_process
    # else:
    #     if not debug:
    #         print(module, 'Refine Command:', subprocess.list2cmdline(refine_cmd))
    ...
    # Extract/Format Sequence Information & Configure AA Dictionaries
    pdb_seq = [None, None]
    errors = [None, None]
    pssm_dict = [None, None]
    pssm_files = [None, None]
    chain = template_pdb.chain_id_list[0]
    for n in range(2):
        pdb_seq[n], errors[n] = fc.extract_aa_seq(template_pdb, chain=chain)
        pssm_dict[n] = populate_design_dict(len(pdb_seq[n]), fc.alph_3_aa_list)
        chain = template_pdb.chain_id_list[-1]
    full_design_dict = populate_design_dict(len(pdb_seq[0] + pdb_seq[1]),
                                            [i for i in range(lower_bound, upper_bound + 1)])

    # Check alignment to assign correct PDB name to each sequence from ASU.pdb file  #GOTIME REMOVE
    alignment1 = pairwise2.align.globalxx(pdb_seq[0], olig_seq1)
    alignment2 = pairwise2.align.globalxx(pdb_seq[0], olig_seq2)
    if alignment1[0][2] >= alignment2[0][2]:
        name = [oligomer1_name, oligomer2_name]
    else:
        name = [oligomer2_name, oligomer1_name]

    # Check to see if other poses have already collected sequence info about design
    if cleaned_pdb not in os.listdir(seq_dir):
        shutil.copy(cleaned_pdb, seq_dir)
        # Check to see if other combinations have already collected sequence info about both design candidates
        seq_files = [None, None]
        for file in os.listdir(design_sequence_dir):
            if not seq_files[0]:
                if fnmatch.fnmatch(file, name[0] + '.hmm'):
                    seq_files[0] = os.path.join(design_sequence_dir, file)
            if not seq_files[1]:
                if fnmatch.fnmatch(file, name[1] + '.hmm'):
                    seq_files[0] = os.path.join(design_sequence_dir, file)

        # Make PSSM of PDB sequence POST-SEQUENCE EXTRACTION
        print(module, 'PSSM: Getting PSSM files...')
        pdb_seq_file = [None, None]
        pssm_process = [None, None]
        for i in range(2):
            if not seq_files[i]:
                pdb_seq_file[i] = fc.write_fasta_file(pdb_seq[i], name[i], outpath=design_sequence_dir)
                if pdb_seq_file[i]:
                    error_string = '[WARNING] Sequence generation ran into the following residue errors: '
                    if errors[i]:
                        print(module, error_string, errors[i])
                else:
                    print(module, '[ERROR] Unable to parse sequence files. '
                                  'Check input PDB is valid. \nProgram Termination')
                    sys.exit()

                pssm_files[i], pssm_process[i] = make_pssm_hhblits(name[i], outpath=design_sequence_dir)
            else:
                pssm_files[i] = os.path.join(seq_dir, os.path.basename(seq_files[i]))
                pssm_process[i] = done_process
                shutil.copy(seq_files[i], pssm_files[i])
        # WAIT for PSSM command to complete
        for n in range(2):
            pssm_process[n].communicate()

    else:
        for i in range(2):
            pssm_files[i] = os.path.join(seq_dir, name[i] + '.hmm')
    ...
    for n in range(2):
        pssm_dict[n] = parse_hhblits_pssm(pssm_files[n], pssm_dict[n])
    full_pssm = combine_pssm(pssm_dict[0], pssm_dict[1])  # chain1 (smaller oligomer) first then chain2 (bigger)

    # Make single PSSM with cluster fragments, and evolutionary PSSM
    dssm = combine_ssm(full_pssm, final_issm, residue_cluster_map, weights=True)  # final_issm is 0 indexed
    dssm_file = make_pssm_file(dssm, os.path.splitext(cleaned_pdb)[0])

    # WAIT for Rosetta Refine command to complete, then move on
    refine_process.communicate()
    # DESIGN: Prepare Command, and Flags file then run RosettaScripts on Refined ASU #GOTIME REMOVE
    if not debug:
        design_variables = [('interface_separator', jump), ('pdb_reference', cleaned_pdb), ('dist', dist),
                            ('pssm_file', dssm_file), ('interface', ','.join(str(i) for i in int_res_numbers)),
        ...
        flagdesign = prepare_rosetta_flags(design_options, design_variables, design_stage[1])
    ...
    # # DESIGN: Prepare Command, and Flags file then run RosettaScripts on Refined ASU #GOTIME
    # design_variables = [('interface_separator', jump), ('pdb_reference', cleaned_pdb), ('dist', dist),
    #                     ('pssm_file', dssm_file), ('interface', ','.join(str(i) for i in int_res_numbers)),
    #                     ('interface1', ','.join(str(i) for i in int_res_numbers1)),
    #                     ('interface2', ','.join(str(i) for i in int_res_numbers2)), ('cst_value', cst_value)]
    # # 'fix_res', residues_to_fix
    # flagdesign = prepare_rosetta_flags(design_options, design_variables, design_stage[1])
    # input_design_cmds = ['-in:file:s', refined_pdb, '-in:file:native', cleaned_pdb, '@' + flagdesign,
    #                      '-parser:protocol', protocol[1]]
    # design_cmd = prepare_rosetta_scripts_cmd(input_design_cmds)
    # if script:
    #     write_shell_script(subprocess.list2cmdline(design_cmd), i=1)
    # else:
    #     if not debug:
    #         print(module, 'Design Command:', subprocess.list2cmdline(design_cmd))
    #         subprocess.call(design_cmd)
