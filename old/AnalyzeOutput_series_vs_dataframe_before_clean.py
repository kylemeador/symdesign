...
# Globals
metric_master = {'buns_heavy_total': 'Buried unsaturated hydrogen bonding heavy atoms in the pose',
                 'buns_hpol_total': 'Buried unsaturated hydrogen bonding hydrogen atoms in the pose',
                 ...
                 'int_area_hydrophobic': 'Total interface solvent accessible surface area hydrophobic',
                 'int_area_polar': 'Total interface solvent accessible surface area polar',
...
rename_columns = {'int_sc': 'shape_complementarity', 'int_sc_int_area': 'int_area',  # 'total_score': 'REU',
                  ...
                  'relax_switch': 'protocol', 'no_constraint_switch': 'protocol',  # 'decoy': 'design',
...
protocols = ['limit_to_profile_switch', 'favor_profile_switch', 'combo_profile_switch', 'no_constraint_switch']
protocol_specific_columns = ['int_energy_res_summary_delta', 'int_energy_context_delta', 'shape_complementarity',
                             'buns_total', 'contact_count', 'interaction_energy_complex', 'int_area_hydrophobic',
                             'int_area_polar', 'int_area_total', 'number_hbonds']
                             # , 'protocol']
#                            'full_stability_delta',
...
def read_scores(file):
    """Take a json formatted score.sc file and incorporate into dictionary
...
def calc_relative_sasa(aa, sasa):  # , sidechain=False):
    ...
    return round(sasa / gxg_sasa[aa], 2)
...
def hbond_processing(score_dict, columns):
    ...
    Args:
        ...
        columns (list): [hbonds_res_selection_complex, hbonds_res_selection_A_oligomer, hbonds_res_selection_B_oligomer]
    Returns:
...
def residue_processing(score_dict, mutations, chain_offset, hbonds=None):
    ...
    dict_template = {'aa': None, 'energy': {'complex': 0, 'oligomer': 0, 'fsp': 0, 'cst': 0},
                     'sasa': {'polar': {'complex': 0, 'oligomer': 0}, 'hydrophobic': {'complex': 0, 'oligomer': 0},
                              'total': {'complex': 0, 'oligomer': 0}}, 'hbond': 0, 'core': 0, 'rim': 0, 'support': 0}
    #                , 'hot_spot': 0}
    total_residue_dict = {}
    for entry in score_dict:
        residue_dict = {}
...
def gather_fragment_metrics(_des_dir):
    ...
    Returns:
        metrics_dict (dict): {'nanohedra_score': , 'average_fragment_z_score': , 'unique_fragments': }
    """
...
def df_permutation_test(grouped_df, diff_s, group1_size=0, compare='mean', permutations=1000):
...
def hydrophobic_collapse_index(sequence):
...
def analyze_output(des_dir, delta_refine=False, merge_residue_data=False, debug=False, save_trajectories=True):
    ...
    _rename_columns.update({'R_int_sc': 'shape_complementarity', 'R_full_stability': 'full_stability_complex'})
    ...
    logger.debug('Design indices: %s' % indices)
    # Modify protocol name for refine and consensus
    for stage in PUtils.stage:
        if PUtils.stage[stage] in indices:
            # change value of row PUtils.stage[i], column 'protocol' to PUtils.stage[i] (for consensus and refine)
            strings_df.at[PUtils.stage[stage], 'protocol'] = PUtils.stage[stage]  # TODO remove in future scripts
    logger.debug(strings_df)
    # replace any empty strings with numpy.notanumber
    ...
    # Remove Rosetta score terms as the are unnecessary TODO learn know how to produce them
    ...
    # returns multi-index column dataframe with first column equal to residue number, second equal to residue metric
    ...
    logger.info('Unique Protocols: %s' % str(unique_protocols))
    ...
    for protocol in unique_protocols:
        designs_by_protocol[protocol] = strings_df.index[strings_df['protocol'] == protocol].tolist()
        sequences_by_protocol[protocol] = {chain: {name: all_design_sequences[chain][name]
                                           ...
                                           for chain in all_design_sequences}
        # logger.debug(sequences_by_protocol[protocol])
        res_profile = Ams.analyze_mutations(des_dir, sequences_by_protocol[protocol])
        for key in per_res_keys:
            stats_by_protocol[protocol][key + '_per_res'] = per_res_metric(res_profile, key=key)
            # {protocol: 'jsd_per_res': 0.747, 'int_jsd_per_res': 0.412}, ...}

    # Prepare/Merge processed dataframes
    ...
    strings_df.columns = pd.MultiIndex.from_product([[''], strings_df.columns])  # shift up or down with merge needs
    residue_df = pd.merge(strings_df, residue_df, left_index=True, right_index=True)

    # Drop refine row and any rows with nan values
    scores_df = scores_df.drop(PUtils.stage[1], axis=0)
    residue_df = residue_df.drop(PUtils.stage[1], axis=0)
    # print(scores_df)
    # print(scores_df.isna().any())  # scores_df.where()
    scores_df.dropna(inplace=True)  # TODO TEST on moved pose pdb versus all_scores.sc
    residue_df.dropna(inplace=True)
    # print(scores_df)

    # POSE ANALYSIS: get total pose statistics
    trajectory_df = scores_df.append(scores_df.mean().rename('average'))
    trajectory_df = trajectory_df.append(scores_df.std().rename('std_dev'))
    protocol_mean_df = scores_df.groupby('protocol').mean()
    protocol_std_df = scores_df.groupby('protocol').std()
    # Use below if scores is Multi-index. Above for single index
    ...
    # Find all unique combinations of protocols
    for pair in combinations(protocol_mean_df.index, 2):
        select_df = protocol_subset_df.loc[designs_by_protocol[pair[0]] + designs_by_protocol[pair[1]], :]
        difference_s = protocol_mean_df.loc[pair[0], protocol_specific_columns].sub(
            protocol_mean_df.loc[pair[1], protocol_specific_columns])
    ...
    # Save Trajectory and Residue dataframes
    if save_trajectories:
        ...
        trajectory_df.to_csv(trajectory_outpath)
        residue_df.to_csv(residue_outpath)
        with open(os.path.join(des_dir.all_scores, str(des_dir) + '_sequences.pkl'), 'wb') as f:
            pickle.dump(all_design_sequences, f, pickle.HIGHEST_PROTOCOL)
    ...
    # Interface B Factor TODO make sure clean_asu.pdb has B-factors included
    ...
    # other_metrics_df = pd.DataFrame(other_pose_metrics, index=[str(des_dir), ])
    # other_metrics_df = pd.concat(other_metrics_df, axis=1, keys=['pose'])

    # CONSTRUCT: Create pose series and format index names
    pose_s = trajectory_df.loc['average', :]  # .rename(str(des_dir))
    # Collect protocol specific metrics in series
    protocol_ave_s = pd.concat([protocol_subset_df.loc[protocol, :] for protocol in unique_protocols],
                               keys=unique_protocols)  # .rename(str(des_dir))
    protocol_stdev_s = pd.concat([protocol_subset_df.loc[protocol + '_stdev', :] for protocol in unique_protocols],
                                 keys=unique_protocols)  # .rename(str(des_dir))
    protocol_stats_s = pd.concat([pd.series(stats_by_protocol[protocol]) for protocol in stats_by_protocol],
                                 keys=unique_protocols)  # .rename(str(des_dir))
    other_metrics_s = pd.Series(other_pose_metrics)  # , name=str(des_dir))
    # Find the similarity between each protocol, take the average of all p-values measured
    similarity_s = pvalue_df.mean().rename(str(des_dir))
    # pose_df = pd.DataFrame(trajectory_df.loc['average', :]).T
    # protocol_ave_df = pd.DataFrame(protocol_ave_s).T
    # protocol_stdev_df = pd.DataFrame(protocol_stdev_s).T
    # protocol_stats_df = pd.DataFrame(protocol_stats_s).T
    # similarity_df = pd.DataFrame(pvalue_df.mean()).T

    # Add index to dataframes
    # pose_df.index = [str(des_dir), ]
    # similarity_df.index = similarity_df.index[str(des_dir), ]

    # Add series specific Multi-index names to data
    pose_s = pd.concat([pose_s], keys=['pose'])
    pose_s = pd.concat([pose_s], keys=['average'])
    protocol_ave_s = pd.concat([protocol_ave_s], keys=['average'])
    protocol_stdev_s = pd.concat([protocol_stdev_s], keys=['stdev'])
    protocol_stats_s = pd.concat([protocol_stats_s], keys=['sequence'])
    other_metrics_s = pd.concat([other_metrics_s], keys=['pose'])
    other_metrics_s = pd.concat([other_metrics_s], keys=['dock'])
    similarity_s.index = pd.MultiIndex.from_tuples(similarity_s.index)
    similarity_s = pd.concat([similarity_s], keys=['similarity']).swaplevel(0, 2)
    # protocol_ave_df = pd.concat(protocol_ave_df, axis=1, keys=['average'])
    # protocol_stdev_df = pd.concat(protocol_stdev_df, axis=1, keys=['stdev'])
    # protocol_stats_df = pd.concat(protocol_stats_df, axis=1, keys=['sequence'])
    # other_metrics_df = pd.concat(other_metrics_df, axis=1, keys=['dock'])
    # similarity_df.columns = pd.MultiIndex.from_tuples(similarity_df.columns)
    # similarity_df.columns = pd.MultiIndex.from_product([similarity_df.columns, ['similarity']])
    # similarity_df = pd.concat([similarity_df], axis=1, keys=['similarity']).swaplevel(0, 2, 1)

    # # Add Protocol specific metrics to pose_df
    # protocol_specific_dfs = []
    # for protocol in unique_protocols:
    #     temp_df = pd.DataFrame(protocol_subset_df.loc[protocol, :]).T
    ...
    #     for stat in stats_by_protocol[protocol]:
    #         temp_df[stat] = stats_by_protocol[protocol][stat]
    #     temp_df.columns = pd.MultiIndex.from_product([[protocol], temp_df.columns])
    #     protocol_specific_dfs.append(temp_df)
    # # combine all pose specific dataframes along the columns axis
    # protocol_specific_df = pd.concat(protocol_specific_dfs, axis=1)

    # Combine all series
    pose_s = pd.concat([pose_s, protocol_ave_s, protocol_stdev_s, protocol_stats_s, other_metrics_s, similarity_s])
    pose_s = pose_s.swaplevel(0, 1).rename(str(des_dir))
    # pose_df = pd.concat([pose_df, protocol_ave_df, protocol_stdev_df, protocol_stats_df], axis=1)
    # pose_df = pd.merge(pose_df, other_metrics_df, left_index=True, right_index=True)
    # pose_df = pd.merge(pose_df, protocol_specific_df, left_index=True, right_index=True)
    # pose_df = pd.merge(pose_df, similarity_df, left_index=True, right_index=True)

    # Remove pose specific metrics from pose_s
    remove_pose_columns = ['protocol', ]
    pose_s = pose_s.drop(remove_pose_columns, level=2)
    # pose_df = pd.DataFrame(pose_s).T
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--multi_processing', action='store_true', help='Should job be run with multiprocessing? '
                                                                              'Default=False')
    parser.add_argument('-s', '--no_save', action='store_false', help='Don\'t save trajectory information? '
                                                                      'Default=False')
    ...
    if args.multi_processing:
        ...
        for des_directory in all_designs:
            pose_results.append(analyze_output(des_directory, delta_refine=args.delta_g, merge_residue_data=args.join,
                                               debug=args.debug, save_trajectories=args.no_save))
