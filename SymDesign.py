...
if __name__ == '__main__':
    ...
    # --------------------------------------------------- # TODO v move to AnalyzeMutatedSequence.py
        ...
        if args.global_sequences:
            ...
            logger.info([design for df in all_dfs[:3] for design in df.index.to_list() if design_directories[idx].name in design])
            logger.info([df.index for df in all_dfs[:3]])
            df = pd.concat([df.drop([design for design in df.index.to_list() if design_directories[idx].name in design],
                                    inplace=True) for idx, df in enumerate(all_dfs)],
