...
def design(des_dir, frag_db, sym, script=False, mpi=False, suspend=False, debug=False):
    ...
    # logger.info('Interface Residues: %s' % ', '.join(str(n) for name in int_res_numbers for n in int_res_numbers[name]))
    ...
    if PUtils.clean not in os.listdir(des_dir.building_blocks):
        ...
        for name in names:
            for seq_file in glob.iglob(os.path.join(des_dir.sequences, name + '.*')):
    ...
    if script:
        ...
        # SDUtils.write_shell_script(subprocess.list2cmdline(consensus_cmd), stage=5, outpath=des_dir.path)
    else:
        if not suspend:
    ...
    design_cmd = main_cmd + \
        ['-in:file:s', refined_pdb, '-in:file:native', cleaned_pdb, '-nstruct', str(CUtils.nstruct),
    ...
    if script:
        ...
        SDUtils.write_shell_script(subprocess.list2cmdline(metric_cmds[pdb_codes[0]]), stage=3, outpath=des_dir.path,
                                   additional=subprocess.list2cmdline(metric_cmds[pdb_codes[1]]))  # TODO >2 chains
    else:
        if not suspend:
    ...
    if script:
        ...
        SDUtils.write_shell_script(analysis_cmd, stage=4, outpath=des_dir.path)
    else:
        if not suspend:
...
if __name__ == '__main__':
    ...
    if args.mpi:
        ...
        logger.info('Setting job up for submission to MPI capable computer. Pose trajectories will run in parallel, '
                    ...
                    (CUtils.mpi - 1, CUtils.nstruct / CUtils.mpi - 1))
    ...
    if args.multi_processing:
        ...
        results, exceptions = SDUtils.old_mp_starmap(design, zipped_args, mp_threads)
        if exceptions:
        ...
        for des_directory in all_design_dirs:
            design(des_directory, args.fragment_database, args.symmetry_group, script=args.command_only,
                   mpi=args.mpi, suspend=args.suspend, debug=args.debug)  # fsp=args.prioritize_frags,
    ...
    if args.command_only:
        ...
        for i, stage in enumerate(PUtils.stage):
            ...
            logger.info('All \'%s\' commands were written to \'%s\'' % (PUtils.stage[stage].title(), command_files[i]))
        logger.info('\nTo process all commands in correct order, execute:\ncd %s\n%s\n' %
                    (args.directory, 'bash ' + PUtils.process_commands + extras))
