...
import PDB
from SymDesignUtils import get_all_pdb_paths
...
def orient_oligomer(pdb_path, sym, out_dir=os.getcwd(), orient_dir=orient_dir):
    ...
    oriented_pdb.write(os.path.join(out_dir, os.path.basename(os.path.splitext(filename)[0]) + '_oriented.pdb')

    return os.path.join(out_dir, os.path.basename(os.path.splitext(filename)[0]) + '_oriented.pdb'
