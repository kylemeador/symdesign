...
class GhostFragment:
    def __init__(self, pdb, i_frag_type, j_frag_type, k_frag_type, ghostfrag_central_res_tup, aligned_surf_frag_central_res_tup, guide_atoms=None, guide_coords=None, pdb_coords=None):
        self.pdb = pdb
...
class MonoFragment:
    def __init__(self, pdb, monofrag_cluster_rep_dict=None, type=None, guide_coords=None, central_res_num=None, central_res_chain_id=None, pdb_coords=None, rmsd_thresh=0.75):
        self.pdb = None
        ...
        if monofrag_cluster_rep_dict is None and type is not None and guide_coords is not None and central_res_num is not None and central_res_chain_id is not None and pdb_coords is not None:
            ...
            self.type = type
            self.guide_coords = guide_coords
            a1 = Atom(1, "CA", " ", "GLY", "9", 0, " ", guide_coords[0][0], guide_coords[0][1], guide_coords[0][2], 1.00, 20.00, "C", "")
            a2 = Atom(2, "N", " ", "GLY", "9", 0, " ", guide_coords[1][0], guide_coords[1][1], guide_coords[1][2], 1.00, 20.00, "N", "")
            a3 = Atom(3, "O", " ", "GLY", "9", 0, " ", guide_coords[2][0], guide_coords[2][1], guide_coords[2][2], 1.00, 20.00, "O", "")
                ...
                for dirpath2, dirnames2, filenames2 in os.walk(dirpath1):
                    for filename in filenames2:
                        if filename.endswith(".pdb"):
                            ...
                            ijk_frag_cluster_rep_mapped_chain_id = filename[filename.find("mappedchain") + 12:filename.find("mappedchain") + 13]
                            ijk_frag_cluster_rep_partner_chain_id = filename[filename.find("partnerchain") + 13:filename.find("partnerchain") + 14]
                            ...
                            i_j_k_intfrag_cluster_rep_dict[i_cluster_type][j_cluster_type][k_cluster_type] = (ijk_frag_cluster_rep_pdb, ijk_frag_cluster_rep_mapped_chain_id, intfrag_mapped_chain_central_res_num, ijk_frag_cluster_rep_partner_chain_id, intfrag_partner_chain_central_res_num)
                ...
                for dirpath2, dirnames2, filenames2 in os.walk(dirpath1):
                    for filename in filenames2:
                        if filename.endswith(".txt"):
                            intfrag_cluster_info_dict[i_cluster_type][j_cluster_type][k_cluster_type] = ClusterInfoFile(dirpath1 + "/" + filename)
